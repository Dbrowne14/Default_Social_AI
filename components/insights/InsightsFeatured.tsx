...
const InsightsFeatured = () => {
  ...
  return (
    <section className="py-20" data-screen-label="Featured">
      <div className="container-custom">
        <article className="grid grid-cols-1 md:grid-cols-2 gap-0 md:gap-14 items-stretch border border-line rounded-[20px] overflow-hidden [background:linear-gradient(180deg,var(--ink-2),var(--ink))]">
          <div className="min-h-120 relative dlex flex-column justify-end p-9 [background:radial-gradient(circle_at_30%_30%,color-mix(in_oklch,var(--accent)_25%,var(--ink-3)),transparent_55%),radial-gradient(circle_at_70%_70%,color-mix(in_oklch,var(--accent)_12%,var(--ink-3)),transparent_60%),repeating-linear-gradient(135deg,var(--ink-2)_0_14px,var(--ink-3)_14px_28px)]">
          ...
          <div className="p-14 flex flex-col gap-6">
            ...
            <h2 className="font-serif text-[clamp(36px,4.6vw,64px)]">
              {featuredArticle?.title}
            </h2>
            <p className="text-cream-2 text-[17px] leading-[1.55] max-w-[52ch]">
              {featuredArticle?.excerpt}
            </p>
            <div className="flex items-center gap-3 mt-2">
              <div className="w-9 h-9 rounded-[50%] bg-accent text-ink font-serif text-[18px] flex items-center justify-center">
